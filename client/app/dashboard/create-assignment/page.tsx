  ...
  const handleGenerateContent = async () => {
    ...
    try {
      for (const output of outputs) {
        ...
        switch (output) {
          case "instructions":
            prompt = `Create assignment instructions for a "${ASSIGNMENT_TYPE_INFO[selectedType].title}" on the topic "${assignmentTitle}" in the course "${selectedCourse}". Learning objectives: ${learningObjectives}.`
            break
          case "rubric":
            prompt = `Create a grading rubric for a "${ASSIGNMENT_TYPE_INFO[selectedType].title}" on "${assignmentTitle}". Include clear criteria and point breakdowns.`
            break
          case "questions":
            prompt = `Generate 10 multiple-choice questions for the topic "${assignmentTitle}" in the course "${selectedCourse}".`
            break
          case "answer_key":
            prompt = `Provide answers and explanations for the multiple-choice questions on "${assignmentTitle}".`
            break
          case "checklist":
            prompt = `Create a lab report checklist for the topic "${assignmentTitle}" in "${selectedCourse}".`
            break
          case "participation_criteria":
            prompt = `Write discussion participation criteria for a classroom discussion on "${assignmentTitle}".`
            break
          case "peer_evaluation":
            prompt = `Create a peer evaluation form for a group project on "${assignmentTitle}".`
            break
          default:
            continue
        }
        const res: any = await postRequest("/assignment/generate", {
          prompt
        });
        newContent[output] = res.result;
      }
      ...
      toast({
        ...
        description: "Your assignment content has been generated using AI",
      })
    } catch (err) {
      console.error("Error generating content:", err)
      toast({
        title: "Error",
        description: "Failed to generate assignment content",
        variant: "destructive"
      })
    } finally {
...
//   }
  ...
  const handleSaveAssignment = async () => {
    setIsSaving(true)
    try {
      const saveResponse = await postRequest("/assignment/save", {
        assignmentTitle,
        selectedCourse,
        selectedType,
        learningObjectives,
      ...
      toast({
        title: "Error",
        description: "Failed to generate and save assignment content",
        variant: "destructive",
      });
    } finally {
      setIsSaving(false)
